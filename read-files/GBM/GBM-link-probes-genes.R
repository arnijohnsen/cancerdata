...
load("../Rdata/GBM/info/GBM-probes.Rdata")
load("../Rdata/GBM/info/GBM-genes.Rdata")

# Filter probe.annotation with probes in use
probe.annotation <- probe.annotation[probe.annotation$TargetID %in% GBM.probes,]
...
idx <- genes.loop %in% GBM.genes

# Save data to file and exit
GBM.linked.probes.genes <- data.frame(probes = probe.loop[idx], genes = genes.loop[idx])
save(GBM.linked.probes.genes, file="../Rdata/GBM/info/GBM-linked-probes-genes.Rdata")
quit(save="no")
