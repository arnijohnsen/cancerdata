...
load("../Rdata/OV/info/OV-probes.Rdata")
load("../Rdata/OV/info/OV-genes.Rdata")

# Filter probe.annotation with probes in use
probe.annotation <- probe.annotation[probe.annotation$TargetID %in% OV.probes,]
...
idx <- genes.loop %in% OV.genes

# Save data to file and exit
OV.linked.probes.genes <- data.frame(probes = probe.loop[idx], genes = genes.loop[idx])
save(OV.linked.probes.genes, file="../Rdata/OV/info/OV-linked-probes-genes.Rdata")
quit(save="no")
