...
load("../Rdata/READ/info/READ-probes.Rdata")
load("../Rdata/READ/info/READ-genes.Rdata")

# Filter probe.annotation with probes in use
probe.annotation <- probe.annotation[probe.annotation$TargetID %in% READ.probes,]
...
idx <- genes.loop %in% READ.genes

# Save data to file and exit
READ.linked.probes.genes <- data.frame(probes = probe.loop[idx], genes = genes.loop[idx])
save(READ.linked.probes.genes, file="../Rdata/READ/info/READ-linked-probes-genes.Rdata")
quit(save="no")
