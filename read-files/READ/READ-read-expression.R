...
data.file.dir = "../rawdata/READ/RNASeq/RNASeqV2/UNC__IlluminaHiSeq_RNASeqV2/Level_3/"
file.sample.map <- read.table("../rawdata/READ/RNASeq/FILE_SAMPLE_MAP.txt", header=T, sep="\t")
...
if(contains.normal){
  READ.CEA <- as.data.frame(t(cancer.rpkm.values)[,gg.normal & gg.cancer])
  READ.NEA <- as.data.frame(t(normal.rpkm.values)[,gg.normal & gg.cancer])
  READ.genes <- colnames(READ.CEA)
  READ.CEA.samples <- rownames(READ.CEA)
  READ.NEA.samples <- rownames(READ.NEA)
  save(READ.CEA,         file="../Rdata/READ/data/READ-CEA.Rdata")
  save(READ.NEA,         file="../Rdata/READ/data/READ-NEA.Rdata")
  save(READ.genes,      file="../Rdata/READ/info/READ-genes.Rdata")
  save(READ.CEA.samples, file="../Rdata/READ/info/READ-CEA-samples.Rdata")
  save(READ.NEA.samples, file="../Rdata/READ/info/READ-NEA-samples.Rdata")
}else{
  READ.CEA <- as.data.frame(t(cancer.rpkm.values)[,gg.cancer])
  READ.genes <- colnames(READ.CEA)
  READ.CEA.samples <- rownames(READ.CEA)
  save(READ.CEA,         file="../Rdata/READ/data/READ-CEA.Rdata")
  save(READ.genes,      file="../Rdata/READ/info/READ-genes.Rdata")
