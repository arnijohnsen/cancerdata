...
load("../Rdata/LUSC/info/LUSC-probes.Rdata")
load("../Rdata/LUSC/info/LUSC-genes.Rdata")

# Filter probe.annotation with probes in use
probe.annotation <- probe.annotation[probe.annotation$TargetID %in% LUSC.probes,]
...
idx <- genes.loop %in% LUSC.genes

# Save data to file and exit
LUSC.linked.probes.genes <- data.frame(probes = probe.loop[idx], genes = genes.loop[idx])
save(LUSC.linked.probes.genes, file="../Rdata/LUSC/info/LUSC-linked-probes-genes.Rdata")
quit(save="no")
