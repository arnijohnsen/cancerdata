...
load("../Rdata/COAD/info/COAD-probes.Rdata")
load("../Rdata/COAD/info/COAD-genes.Rdata")

# Filter probe.annotation with probes in use
probe.annotation <- probe.annotation[probe.annotation$TargetID %in% COAD.probes,]
...
idx <- genes.loop %in% COAD.genes

# Save data to file and exit
COAD.linked.probes.genes <- data.frame(probes = probe.loop[idx], genes = genes.loop[idx])
save(COAD.linked.probes.genes, file="../Rdata/COAD/info/COAD-linked-probes-genes.Rdata")
quit(save="no")
