...
load("../Rdata/PRAD/info/PRAD-probes.Rdata")
load("../Rdata/PRAD/info/PRAD-genes.Rdata")

# Filter probe.annotation with probes in use
probe.annotation <- probe.annotation[probe.annotation$TargetID %in% PRAD.probes,]
...
idx <- genes.loop %in% PRAD.genes

# Save data to file and exit
PRAD.linked.probes.genes <- data.frame(probes = probe.loop[idx], genes = genes.loop[idx])
save(PRAD.linked.probes.genes, file="../Rdata/PRAD/info/PRAD-linked-probes-genes.Rdata")
quit(save="no")
