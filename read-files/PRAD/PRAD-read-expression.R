...
data.file.dir = "../rawdata/PRAD/RNASeq/RNASeqV2/UNC__IlluminaHiSeq_RNASeqV2/Level_3/"
file.sample.map <- read.table("../rawdata/PRAD/RNASeq/FILE_SAMPLE_MAP.txt", header=T, sep="\t")
...
if(contains.normal){
  PRAD.CEA <- as.data.frame(t(cancer.rpkm.values)[,gg.normal & gg.cancer])
  PRAD.NEA <- as.data.frame(t(normal.rpkm.values)[,gg.normal & gg.cancer])
  PRAD.genes <- colnames(PRAD.CEA)
  PRAD.CEA.samples <- rownames(PRAD.CEA)
  PRAD.NEA.samples <- rownames(PRAD.NEA)
  save(PRAD.CEA,         file="../Rdata/PRAD/data/PRAD-CEA.Rdata")
  save(PRAD.NEA,         file="../Rdata/PRAD/data/PRAD-NEA.Rdata")
  save(PRAD.genes,      file="../Rdata/PRAD/info/PRAD-genes.Rdata")
  save(PRAD.CEA.samples, file="../Rdata/PRAD/info/PRAD-CEA-samples.Rdata")
  save(PRAD.NEA.samples, file="../Rdata/PRAD/info/PRAD-NEA-samples.Rdata")
}else{
  PRAD.CEA <- as.data.frame(t(cancer.rpkm.values)[,gg.cancer])
  PRAD.genes <- colnames(PRAD.CEA)
  PRAD.CEA.samples <- rownames(PRAD.CEA)
  save(PRAD.CEA,         file="../Rdata/PRAD/data/PRAD-CEA.Rdata")
  save(PRAD.genes,      file="../Rdata/PRAD/info/PRAD-genes.Rdata")
