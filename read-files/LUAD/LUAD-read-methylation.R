...
data.file.dir = "../rawdata/LUAD/Methylation/DNA_Methylation/JHU_USC__HumanMethylation450/Level_3/"
file.sample.map <- read.table("../rawdata/LUAD/Methylation/FILE_SAMPLE_MAP.txt", header=T, sep="\t")
...
if(contains.normal){
  LUAD.CMP <- as.data.frame(t(cancer.beta.values)[,gg.normal & gg.cancer])
  LUAD.NMP <- as.data.frame(t(normal.beta.values)[,gg.normal & gg.cancer])
  LUAD.probes <- colnames(LUAD.CMP)
  LUAD.CMP.samples <- rownames(LUAD.CMP)
  LUAD.NMP.samples <- rownames(LUAD.NMP)
  save(LUAD.CMP,         file="../Rdata/LUAD/data/LUAD-CMP.Rdata")
  save(LUAD.NMP,         file="../Rdata/LUAD/data/LUAD-NMP.Rdata")
  save(LUAD.probes,      file="../Rdata/LUAD/info/LUAD-probes.Rdata")
  save(LUAD.CMP.samples, file="../Rdata/LUAD/info/LUAD-CMP-samples.Rdata")
  save(LUAD.NMP.samples, file="../Rdata/LUAD/info/LUAD-NMP-samples.Rdata")
}else{
  LUAD.CMP <- as.data.frame(t(cancer.beta.values)[,gg.cancer])
  LUAD.probes <- colnames(LUAD.CMP)
  LUAD.CMP.samples <- rownames(LUAD.CMP)
  save(LUAD.CMP,         file="../Rdata/LUAD/data/LUAD-CMP.Rdata")
  save(LUAD.probes,      file="../Rdata/LUAD/info/LUAD-probes.Rdata")
