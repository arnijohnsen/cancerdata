...
data.file.dir = "../rawdata/LUAD/RNASeq/RNASeqV2/UNC__IlluminaHiSeq_RNASeqV2/Level_3/"
file.sample.map <- read.table("../rawdata/LUAD/RNASeq/FILE_SAMPLE_MAP.txt", header=T, sep="\t")
...
if(contains.normal){
  LUAD.CEA <- as.data.frame(t(cancer.rpkm.values)[,gg.normal & gg.cancer])
  LUAD.NEA <- as.data.frame(t(normal.rpkm.values)[,gg.normal & gg.cancer])
  LUAD.genes <- colnames(LUAD.CEA)
  LUAD.CEA.samples <- rownames(LUAD.CEA)
  LUAD.NEA.samples <- rownames(LUAD.NEA)
  save(LUAD.CEA,         file="../Rdata/LUAD/data/LUAD-CEA.Rdata")
  save(LUAD.NEA,         file="../Rdata/LUAD/data/LUAD-NEA.Rdata")
  save(LUAD.genes,      file="../Rdata/LUAD/info/LUAD-genes.Rdata")
  save(LUAD.CEA.samples, file="../Rdata/LUAD/info/LUAD-CEA-samples.Rdata")
  save(LUAD.NEA.samples, file="../Rdata/LUAD/info/LUAD-NEA-samples.Rdata")
}else{
  LUAD.CEA <- as.data.frame(t(cancer.rpkm.values)[,gg.cancer])
  LUAD.genes <- colnames(LUAD.CEA)
  LUAD.CEA.samples <- rownames(LUAD.CEA)
  save(LUAD.CEA,         file="../Rdata/LUAD/data/LUAD-CEA.Rdata")
  save(LUAD.genes,      file="../Rdata/LUAD/info/LUAD-genes.Rdata")
