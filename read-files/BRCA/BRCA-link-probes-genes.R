...
load("../Rdata/BRCA/info/BRCA-probes.Rdata")
load("../Rdata/BRCA/info/BRCA-genes.Rdata")

# Filter probe.annotation with probes in use
probe.annotation <- probe.annotation[probe.annotation$TargetID %in% BRCA.probes,]
...
idx <- genes.loop %in% BRCA.genes

# Save data to file and exit
BRCA.linked.probes.genes <- data.frame(probes = probe.loop[idx], genes = genes.loop[idx])
save(BRCA.linked.probes.genes, file="../Rdata/BRCA/info/BRCA-linked-probes-genes.Rdata")
quit(save="no")
