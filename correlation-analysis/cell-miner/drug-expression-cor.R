...
cell.miner.cor <- data.frame(row=numeric(0), col=numeric(0), r=numeric(0), p=numeric(0))
...
for(i in 1:chunks){
  ...
  p     <- pt(t, n-2, lower.tail=T)
  idx   <- (p < 0.01 | p > 0.99)&!is.na(p)

  tmp <- data.frame(which(idx, arr.ind=T), r=r[idx], n=n[idx], p=p[idx])
